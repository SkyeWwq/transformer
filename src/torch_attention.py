# -*- coding: utf-8 -*-
# @Time    : 2020/8/10 7:18 下午
# @Author  : Dawein
# @File    : torch_attention.py
# @Software : PyCharm
...
def scaled_dot_product_attention(query, key, value, attn_mask=None):
    ...
    if attn_mask is not None:
        ...
        scores = scores.masked_fill(attn_mask, -1e7)
...
class MultiHeadAttention(nn.Module):
    ...
    def cache_kv(self, input):
        key = F.linear(input, self.W_k)
        value = F.linear(input, self.W_v)
        return [key, value]
